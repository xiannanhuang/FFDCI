...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.revin=True
        self.revin_layer=RevIN(channel=configs['dim'],output_dim=configs['pred_len'])
        self.leddam=Leddam(configs['dim'],dropout=configs['dropout'])
        ...
        self.Linear_res.weight = nn.Parameter(
                (1 / configs['d_model']) * torch.ones([configs['pred_len'], configs['d_model']])) 
    def forward(self, inp,return_feature=False):

        if self.revin:
            inp=self.revin_layer(inp)
        res,main=self.leddam(inp)
        feature=(main+res).permute(0,2,1)
        ...
        if return_feature:
            return {96:pred},feature.detach().cpu().numpy()
        return {96:pred}
